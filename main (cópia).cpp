...
ImGui::SetNextWindowSize(ImVec2(200, (screenHeight/2)-100));
...
{//PREFABS***************************************************************************************************************************************
ImGui::SetNextWindowPos(ImVec2(4, (screenHeight/2)-40));
ImGui::SetNextWindowSize(ImVec2(200, (screenHeight/2)-100));
ImGui::Begin("Game Prefabs", nullptr  ,   ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse|ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoBringToFrontOnFocus  );
ImGui::BeginChild("##Buttons2");
ImGui::Separator();
//ImGui::EndChild();
//ImGui::BeginChild("SceneList");
 // const auto child_size = ImVec2(310, 280);
 ImVec2 wSize = ImGui::GetWindowSize();
            ...
            if (ImGui::BeginListBox("##draw_list_prefabs", draw_list_size))
            {
             /*   if (ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_DownArrow)))
               {
                    if (current_part_idx < m_entities.size() - 1) { ++current_part_idx; }
                }
                if (ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_UpArrow)))
                {
...
ImGui::Begin("WindowSystem",&showFps,ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoTitleBar );
    ImGui::Text("Mouse ( %f | %f ) Selected (%d) Total of Object %d Application average %.3f ms/frame (%.1f FPS)",(mouse.x-viewBond.x),(mouse.y-viewBond.y),current_node_idx,m_entities.size(), 1000.0f / ImGui::GetIO().Framerate, ImGui::GetIO().Framerate);
ImGui::End();
}
